...
void MotionPlanner::setComDes(const Eigen::VectorXd& comTraj, int cbf_status) // comTraj -> x, y, dx, dy  
{

    double velocityChangeThreshold = 0.03; /* your threshold value */;
    double theta = 0;
    double dTheta = 0;

    double dx = comTraj(3);
    double dy = comTraj(4);
    double velocityChange = std::sqrt(dx * dx + dy * dy);
    ...
    if (velocityChange > velocityChangeThreshold) {
        ...
        theta = theta_prev_;  // Assuming the previous theta is stored in path(7 + offset, 0)
    }
    ...
    double thetaTmp = theta_prev_ + kp*(theta - theta_prev_);



    traj.comDes(0) = comTraj(0);
    traj.comDes(1) = comTraj(1);
    traj.comDes(2) = 0.28; // 0.28; //z traj is defined in plan_traj for the standing, otherwise it is constant 0.28 and all the derivatives are zero
    ...
    traj.comDes(3) = comTraj(3);    
    traj.comDes(4) = comTraj(4);
    traj.comDes(5) = 0;

    // Orientation comes here 
    traj.comDes(8) = 0; //thetaTmp; //theta; // Theta
    traj.comDes(11) = 0; //kp*(theta - theta_prev_); // dTheta

    theta_prev_ = thetaTmp;

    std::cout << "MotionPlanner: theta: " << thetaTmp << "     dTheta: " << 0.001*(theta - theta_prev_) << std::endl;
    // traj.comDes(0) = 0;
    // traj.comDes(1) = 0;
...
void MotionPlanner::planTraj(const StateInfo *state, const KinematicsInfo *kin, ContactEst *con_obj, size_t gait, double phase, size_t ctrlTick, MP * params, const Eigen::MatrixXd & Pr_refined_, size_t agent_id_, size_t gaitDomain_, Eigen::MatrixXd NMPC_sol){
...
//            domLenSec -= (domLenSec>1.0) ? 0.25 : 0;
        ...
        double dt = (1.0/LL_Hz);
        double domLenSec = 200*0.001; // 200 ms for SR // For Event-based LIP: TSOPTTICK*0.001*4;
        ...
        if(con->changeDomain==1){
            ...
            // Marc Raibert foothold selection (similar)
            // ================================================ //
            // std::cout << "footPrintTruncated_ = " << footPrintTruncated_ << std::endl;

            double stepTmp[3] = {0};

            // Old MPC code
            // float des_vel_x = ( NMPC_sol(1) + NMPC_sol(1+4) + NMPC_sol(1+2*4) + NMPC_sol(1+3*4) )/4;
            // float des_vel_y = (NMPC_sol(3) + NMPC_sol(3+4) + NMPC_sol(3+2*4) + NMPC_sol(3+3*4))/4;

            // New MPC code
            float des_vel_x = 0.0, des_vel_y = 0.0;
            for (int j = 0; j < 8; ++j) {
                des_vel_x += NMPC_sol(3 + j * 12);  // Accessing the 4th element for each state set
                des_vel_y += NMPC_sol(4 + j * 12);
            }
