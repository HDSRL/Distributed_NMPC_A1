...
void ExternalComm0::HighLevel(){
	...
	mpc_obj->updateState(HLData0.q, 
                         HLData0.dq,
                         HLData0.ind,
                         HLData0.toePos,
                         HLData1.last_state[1 - 0],
                         HLData1.GRFs_sol_.row(1 - 0),
                         HLData0.domain,
                         motiontime);
	...
	if (HLData0.runMPC)
	{
		...
		HLData0.cbf_active = mpc_obj->getCBF_status();
		HLData0.alpha_COM = mpc_obj->get_alphaCOM();
		HLData0.alpha_GRF = mpc_obj->get_alphaGRF();
		HLData0.MPC_sol_ = mpc_obj->get_MPCsol();
        HLData0.GRFs_sol_.row(0) = mpc_obj->get_GRFsol();
		HLData0.last_state[0] = mpc_obj->get_lastState();
		// HLData0.domain = mpc_obj->getDomain();
		
		HLData0.resetRun = -1;

		std::cout << "RUNNING MPC (motiontime:) " << std::to_string(motiontime) << "\n"; 
...
void ExternalComm0::Calc() {
	...
	std::cout << "RUNNING CALC (motiontime:) " << std::to_string(motiontime) << "\n";
	...
	if ( motiontime >= starttime ){
		if (beginCommand & !softFall){
			...
			if (beginPose == 0){
				loco_obj->set_MPC_DATA( LLData0.alpha_COM,
										LLData0.alpha_GRF, 
                                        LLData0.MPC_sol_, 
                                        LLData0.MPC_data_available,
										LLData0.cbf_active);
...
void ExternalComm1::HighLevel(){
	...
	mpc_obj->updateState( HLData1.q, 
                          HLData1.dq, 
                          HLData1.ind, 
                          HLData1.toePos, 
                          HLData0.last_state[0],
                          HLData0.GRFs_sol_.row(0),
                          HLData1.domain,
                          motiontime);
	...
	if (HLData1.runMPC == 1)
	{
		...
		HLData1.cbf_active = mpc_obj->getCBF_status();
		HLData1.alpha_COM = mpc_obj->get_alphaCOM();
		HLData1.alpha_GRF = mpc_obj->get_alphaGRF();
		HLData1.MPC_sol_ = mpc_obj->get_MPCsol();
        HLData1.GRFs_sol_.row(1) = mpc_obj->get_GRFsol();
		HLData1.last_state[1] = mpc_obj->get_lastState();
...
void ExternalComm1::Calc() {
	...
	if ( motiontime >= starttime ){
		if (beginCommand & !softFall){
			...
			beginPose = (motiontime<endStand) ? 0 : 1;
            // beginPose = 0; // For now, we are not using the trotting controller
			if (beginPose == 0){
				loco_obj->set_MPC_DATA( LLData1.alpha_COM,
										LLData1.alpha_GRF,
                                        LLData1.MPC_sol_, 
                                        LLData1.MPC_data_available,
										LLData1.cbf_active);
				// Calculate the torque
            ...
            LLData1.domain = loco_obj->getDomain();
            LLData1.domain_change = loco_obj->hasDomainChanged();
...
int main(int argc, char *argv[])
{
    ...
    Pstart << 0.0, 0.0, 0.0, -1.0; //, 1.0, -2, 1, -3;	//2.0, -2.0
    ...
    Pobs   <<       3.2,       2,      2,             2,           2,                4,             4,     2.5,       -100,
                    0.9+100,       1+100,     -1+100,             2+100,          -2+100,              0.2+100,            -2+100,    -1.0 + 100,    -0.5 + 100; 
	...
	extComm0.mpc_obj  = std::unique_ptr<NMPC>(new NMPC());

	extComm0.loco_obj->setAgentID(0); //Agent ID = 0
    extComm0.loco_obj->setPstart(Pstart);
    extComm0.loco_obj->setPobs(Pobs);

    extComm0.mpc_obj->init();
	extComm0.mpc_obj->setAgentID(0);    //MPC_Agent1 = new MPC_dist();
    extComm0.mpc_obj->setPstart(Pstart);
    extComm0.mpc_obj->setPobs(Pobs);
    extComm0.mpc_obj->setPobs_real(Pobs_real);
    extComm0.mpc_obj->generateReferenceTrajectory();

	// SECOND AGENT

	extComm1.loco_obj = std::unique_ptr<LocoWrapper>(new LocoWrapper(argc, argv));
	extComm1.mpc_obj  = std::unique_ptr<NMPC>(new NMPC());

	extComm1.loco_obj->setAgentID(1); //Agent ID = 0
    extComm1.loco_obj->setPstart(Pstart);
    extComm1.loco_obj->setPobs(Pobs);

    extComm1.mpc_obj->init();
	extComm1.mpc_obj->setAgentID(1);    //MPC_Agent1 = new MPC_dist();
    extComm1.mpc_obj->setPstart(Pstart);
    extComm1.mpc_obj->setPobs(Pobs);
    extComm1.mpc_obj->setPobs_real(Pobs_real);
    extComm1.mpc_obj->generateReferenceTrajectory();


	agentData[0].alpha_COM.setZero(12,5);
    agentData[1].alpha_COM.setZero(12,5);
	
	agentData[0].alpha_GRF.setZero(12,5);
	agentData[1].alpha_GRF.setZero(12,5);
		
	LoopFunc loop_calc0("calc_loop0", extComm0.dt,3, boost::bind(&ExternalComm0::Calc, &extComm0));
	LoopFunc loop_mpc0("mpc_loop0", 0.010001f,4, boost::bind(&ExternalComm0::HighLevel, &extComm0));
	LoopFunc loop_calc1("calc_loop1", extComm1.dt,5, boost::bind(&ExternalComm1::Calc, &extComm1));
	LoopFunc loop_mpc1("mpc_loop1", 0.010001f,4, boost::bind(&ExternalComm1::HighLevel, &extComm1));
	...
	while ( (extComm1.stop==0) ){
		sleep(0.1);
	};

    // extComm0.mpc_obj->logData();
    // extComm1.mpc_obj->logData();
