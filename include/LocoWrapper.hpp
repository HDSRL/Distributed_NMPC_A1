...
public:
    ...
    void calcTau(const double q[18], const double dq[18], const double R[9], const int force[4], size_t gait, size_t ctrlTick, int* checkrunMPC);
    ...
    // From Basit's Code
    void run_NMPC();
    void getComData(double phaseVar);
    ...
    void getComTrajectory(size_t gait, size_t gaitDomain);
    void getGRFTrajectory(size_t gait, size_t gaitDomain);
    void logMPC_Data();
    ...
    void set_MPC_DATA(Eigen::MatrixXd alpha_COM, Eigen::MatrixXd alpha_GRF,  Eigen::MatrixXd MPC_sol, bool avail, int cbf_status);
    void reset_MPC_DATA();
    Eigen::MatrixXd get_footPrintMtx() { return footPrintTruncated_; }
    int getDomain() {return gaitDomain_; }  // Previously domain_; I am not sure if that worked earlier!
...
private:
    ...
    Eigen::MatrixXd alpha_GRF_traj_e_;
